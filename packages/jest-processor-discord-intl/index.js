 ...
 * @returns {string}
 */
function _processCode(source, filePath, _config) {
...
module.exports = {
   ...
   * @param {string} filePath
   * @param {object} config
   * @returns {import('@jest/types').TransformTypes.TransformResult}
   */
  process(source, filePath, config) {
    const code = _processCode(source, filePath, config);
    return { code, originalCode: source, sourceMapPath: null };
  },
};

...
impl<'a, W: std::io::Write> IntlTypesGenerator<'a, W> {
    pub fn new(
        ...
        Self {
            ...
            source_map: SourceMapBuilder::new(Some(&output_file_path)),
            allow_nullability,
        }
    }
    ...
    pub fn into_sourcemap(self) -> anyhow::Result<String> {
        let mut result = Vec::with_capacity(self.database.messages.len() * 10);
        self.source_map.into_sourcemap().to_writer(&mut result)?;
...
impl<W: std::io::Write> IntlDatabaseService for IntlTypesGenerator<'_, W> {
    ...
    fn run(&mut self) -> Self::Result {
        self.source_map.add_source(&self.source_file_key);
        write!(
            self.output,
        ...
        for message_key in source_message_keys {
                ...
                .and_then(|definition| definition.file_position)
            {
                self.source_map.add_raw(
                    self.output.line as u32,
                    self.output.col as u32,

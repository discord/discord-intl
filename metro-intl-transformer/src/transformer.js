...
class MessageDefinitionsTransformer {
   ...
   * @param {() => string} getPrelude A function that returns a prelude to inject at the start of the transformed file.
   * @param {(importPath: string) => string} createAssetImport A function to generate a require statement for the compiled asset file.
   */
  constructor(messageKeys, localeMap, createAssetImport, getPrelude) {
    this.messageKeys = messageKeys;
    this.localeMap = localeMap;
    this.getPrelude = getPrelude;
    this.createAssetImport = createAssetImport;
  }
  ...
  getLocaleRequireMap() {
    ...
    for (const [locale, importPath] of Object.entries(this.localeMap)) {
      ...
      localeProperties.push(
        `"${locale}": () => ${this.createAssetImport(importPath)}.then((data) => ({default: data}))`,
      );
    }
